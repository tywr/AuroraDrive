...
{
    std::complex<double> result = wrightomega(std::complex<double>(x, 0.0));
    return result.real();
}
...
//      w = wrightomega ( z )
//
//    This function makes a call to the more powerful wrightomega_ext()
//    function.
//
//  Modified:
//
...
int wrightomega_ext(
    complex<double> z, complex<double>& w, complex<double>& e,
    complex<double>& r, complex<double>& cond
)
...
//
{
    ...
    else if (x <= -2.0 && -pi < y && y <= pi)
    {
        pz = exp(z);
        w = (1.0 +
             (-1.0 + (3.0 / 2.0 + (-8.0 / 3.0 + 125.0 / 24.0 * pz) * pz) * pz) *
                 pz) *
            pz;
    }
    //
             ...
             ((-2.0 < x) && (x - 1.0) * (x - 1.0) + y * y <= pi * pi))
    {
        ...
        w = 1.0 / 2.0 + 1.0 / 2.0 * z +
            (1.0 / 16.0 +
             (-1.0 / 192.0 + (-1.0 / 3072.0 + 13.0 / 61440.0 * pz) * pz) * pz) *
                pz * pz;
    }
    //
    //  Region 5: Top wing.
